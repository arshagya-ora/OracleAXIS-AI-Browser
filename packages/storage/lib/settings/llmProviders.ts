...
export interface ProviderConfig {
  ...
  bridgeToken?: string; // Local SSO bridge shared secret
}
...
export function getProviderTypeByProviderId(providerId: string): ProviderTypeEnum {
  switch (providerId) {
    ...
    case CODEX_SSO_PROVIDER_ID:
      return ProviderTypeEnum.CodexSsoBridge;
  ...
  switch (providerId) {
    ...
    case ProviderTypeEnum.CodexSsoBridge:
      return {
        ...
        baseUrl: CODEX_SSO_DEFAULT_BRIDGE_URL,
        bridgeToken: '',
...
export const llmProviderStore: LLMProviderStorage = {
  ...
  async setProvider(providerId: string, config: ProviderConfig) {
    ...
    if (providerType === ProviderTypeEnum.CodexSsoBridge) {
      if (!config.baseUrl?.trim()) {
        throw new Error(`Base URL is required for ${getDefaultDisplayNameFromProviderId(providerId)}`);
      }
      if (!config.bridgeToken?.trim()) {
        throw new Error(`Bridge token is required for ${getDefaultDisplayNameFromProviderId(providerId)}`);
      }
