...
describe('codex oca import', () => {
  it('creates and applies the Codex SSO bridge provider defaults', async () => {
    const storage = await import('@extension/storage');

    const defaultConfig = storage.getDefaultProviderConfig(storage.CODEX_SSO_PROVIDER_ID);
    expect(defaultConfig).toMatchObject({
      apiKey: '',
      baseUrl: storage.CODEX_SSO_DEFAULT_BRIDGE_URL,
      bridgeToken: '',
      modelNames: [storage.CODEX_SSO_DEFAULT_MODEL],
      type: storage.ProviderTypeEnum.CodexSsoBridge,
    });

    const providerConfig = storage.createCodexSsoBridgeProviderConfig({
      baseUrl: 'http://127.0.0.1:14550',
      bridgeToken: 'bridge-token',
    });
    const result = await storage.applyCodexSsoBridgeProvider(providerConfig);

    expect(result.firstImport).toBe(true);

    const providers = await storage.llmProviderStore.getAllProviders();
    expect(providers[storage.CODEX_SSO_PROVIDER_ID]).toMatchObject({
      apiKey: '',
      baseUrl: 'http://127.0.0.1:14550',
      bridgeToken: 'bridge-token',
      modelNames: [storage.CODEX_SSO_DEFAULT_MODEL],
    ...
    expect(plannerModel).toMatchObject({
      ...
      modelName: storage.CODEX_SSO_DEFAULT_MODEL,
      parameters: {
        temperature: 0.7,
        topP: 0.9,
      },
...
describe('codex oca runtime', () => {
  it('routes only the Codex import provider through the responses adapter', async () => {
    ...
    const cases = [
      ...
      },
      {
        ...
        expectedClass: 'CodexSsoBridgeChatModel',
      },
    ];
  ...
  it('calls the local Codex SSO bridge without an API Authorization header', async () => {
    ...
    expect(url).toBe('http://127.0.0.1:14550/responses');
    expect(headers.Authorization).toBeUndefined();
    expect(headers['X-OC-Axis-Bridge-Token']).toBe('bridge-token');
    expect(body).toMatchObject({
      model: storage.CODEX_SSO_DEFAULT_MODEL,
  ...
  it('uses manual JSON extraction for the Codex SSO bridge provider', async () => {
    ...
    const fetchMock = vi.fn().mockResolvedValue(createJsonResponse({ output_text: JSON.stringify(plannerPayload) }));
    vi.stubGlobal('fetch', fetchMock);
    ...
    const plannerAgent = createTestAgent(BaseAgent, plannerOutputSchema, chatModel, storage.CODEX_SSO_PROVIDER_ID);
  ...
  it('does not depend on provider or constructor name to disable bridge structured output', async () => {
    const storage = await import('@extension/storage');
    const { BaseAgent } = await import('../agents/base');
    const { plannerOutputSchema } = await import('../agents/planner');
    const { CodexSsoBridgeChatModel } = await import('../models/codexSsoBridge');
    ...
    Object.defineProperty(chatModel.constructor, 'name', {
      configurable: true,
      value: 'MinifiedModelName',
    });

    const plannerAgent = createTestAgent(BaseAgent, plannerOutputSchema, chatModel, '');
